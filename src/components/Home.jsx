...
function HeroSection() {
  return (
    <section className="pt-28 md:pt-36 pb-14 md:pb-20 overflow-hidden">
      <div className="absolute inset-0 -z-10 opacity-40 bg-[radial-gradient(1000px_circle_at_20%_0%,rgba(249,115,22,0.06),transparent_60%),radial-gradient(800px_circle_at_100%_20%,rgba(244,63,94,0.05),transparent_60%)]" />
      <div className="container mx-auto px-4 grid md:grid-cols-2 gap-8 items-center">
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="relative">
          <div className="aspect-[4/3] rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/[0.03] p-4">
            <div className="h-full w-full rounded-xl bg-neutral-900/50 border border-white/10 grid grid-rows-3 gap-3 p-3">
              <div className="flex items-center gap-2">
                <Badge>Pipeline</Badge>
                <Badge>Automations</Badge>
                <Badge>Analytics</Badge>
              </div>
              <div className="rounded-lg bg-white/5 border border-white/10 h-full" />
...
function LogoStrip() {
  const logos = useMemo(
    () => ['Acme', 'Globex', 'Umbrella', 'Soylent', 'Initech', 'Hooli', 'Vehement'],
    []
  );
  return (
    ...
    <section className="py-14 md:py-20">
      <div className="container mx-auto px-4">
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {items.map((f, i) => (
            <motion.div key={f.title} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }} className="group rounded-xl border border-white/10 bg-white/[0.04] p-5">
    ...
    <section className="py-14 md:py-20 border-t border-neutral-900/80">
      <div className="container mx-auto px-4">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
          {steps.map((s, i) => (
            <motion.div key={s.title} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.06 }} className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
  ...
  return (
    <section className="py-12">
      <div className="container mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-4">
        {data.map((d, i) => (
          <motion.div key={d.k} initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.35, delay: i * 0.05 }} className="rounded-xl border border-white/10 bg-white/[0.03] p-5 text-center">
...
function Testimonials() {
  ...
  useEffect(() => {
    controls.start({ opacity: [0, 1], y: [8, 0], transition: { duration: 0.4 } });
  }, [idx, controls]);
    ...
    <section className="py-14 md:py-20 border-t border-neutral-900/80">
      <div className="container mx-auto px-4">
        ...
        <div className="mt-8 grid md:grid-cols-2 gap-6 items-start">
          <motion.blockquote animate={controls} className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
        ...
        <div className="mt-6 divide-y divide-white/10 border border-white/10 rounded-xl overflow-hidden">
          {qas.map((item) => (
            <div key={item.q} className="bg-white/[0.02]">
              ...
              <motion.div initial={false} animate={{ height: open === item.q ? 'auto' : 0, opacity: open === item.q ? 1 : 0 }} className="px-4 overflow-hidden">
...
function CTABanner() {
  return (
    <section className="py-10 md:py-14">
      <div className="container mx-auto px-4">
        <div className="rounded-2xl border border-white/10 bg-gradient-to-r from-white/[0.06] to-white/[0.03] p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
