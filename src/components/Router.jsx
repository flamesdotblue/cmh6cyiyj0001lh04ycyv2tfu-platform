...
import { ParallaxHeader, fadeInUp } from './Parallax';
...
function PricingPage() {
  ...
  const plans = [
    { name: 'Starter', m: 0, y: 0, features: ['2 active jobs', 'Basic search', 'Email alerts', 'GDPR-friendly consent'], popular: false },
    { name: 'Growth', m: 49, y: 39, features: ['10 active jobs', 'AI resume parsing', 'Team collaboration', 'Stage automations'], popular: true },
    { name: 'Scale', m: 149, y: 129, features: ['Unlimited jobs', 'Advanced analytics', 'SSO', 'Priority support'], popular: false },
  ];

  return (
    <main>
      <ParallaxHeader title="Simple pricing for fast hiring" subtitle="Start free. Scale as your team grows. No hidden fees." />
      <section className="container mx-auto px-4 py-10 md:py-14">
        <div className="flex items-center justify-center">
          <div className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 p-1 text-sm">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((p, i) => (
            <motion.div key={p.name} {...fadeInUp(i * 0.05)} className={'relative rounded-2xl border p-6 md:p-7 ' + (p.popular ? 'border-orange-500/40 bg-white/[0.06] shadow-[0_0_40px_-12px_rgba(249,115,22,0.4)]' : 'border-white/10 bg-white/[0.03]')}>
              {p.popular && (
        ...
        <motion.p {...fadeInUp(0.15)} className="mt-6 text-center text-neutral-400 text-xs">All plans include secure hosting, export tools, and compliance-ready audit logs.</motion.p>
      </section>
    </main>
  );
}
...
function AboutPage() {
  ...
  return (
    <main>
      <ParallaxHeader title="About ATS Flow" subtitle="We build minimal, secure tools that keep recruiting teams in flow." />
      <section className="container mx-auto px-4 py-10 md:py-14">
        <div className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 p-1 text-sm">
          {tabs.map((t) => (
        ...
        <motion.div key={tab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mt-6 max-w-3xl">
          {tab === 'Mission' && (
        ...
        <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-4">
          {[{ l: 'Time-to-hire', v: '-43%' }, { l: 'Parsing accuracy', v: '>92%' }, { l: 'Uptime', v: '99.99%' }].map((s, i) => (
            <motion.div key={s.l} {...fadeInUp(i * 0.05)} className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
...
function ContactPage() {
  ...
  return (
    <main>
      <ParallaxHeader title="Talk to our team" subtitle="Tell us about your hiring goals. We’ll tailor a demo and rollout plan." />
      <section ref={ref} className="container mx-auto px-4 py-10 md:py-14">
        <motion.form onSubmit={onSubmit} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }} className="max-w-2xl space-y-5">
